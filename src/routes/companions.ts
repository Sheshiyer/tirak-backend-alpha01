...
// Apply optional authentication middleware to all routes EXCEPT /all
companions.use('*', (c, next) => {
  if (c.req.path === '/all') return next();
...
companions.get('/all', async (c) => {
  ...
  const sortBy = c.req.query('sortBy') || 'u.created_at';
  ...
  try {
    ...
    // Use prefixed sortBy
    const sortColumn = validSortColumns.includes(sortBy) ? sortBy : 'u.created_at';
